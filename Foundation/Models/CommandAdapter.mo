...
model CommandAdapter "任务接口到设备指令与状态的纯映射"
  OneSatSim.Foundation.Interfaces.OpportunitySignals opportunity annotation(Placement(transformation(extent={{-112,62},{-92,82}})));
  OneSatSim.Foundation.Interfaces.SafetySignals safety annotation(Placement(transformation(extent={{-112,30},{-92,50}})));
  OneSatSim.Foundation.Interfaces.ArbiterDecisionSignals decision annotation(Placement(transformation(extent={{-112,-2},{-92,18}})));
  OneSatSim.Foundation.Interfaces.MissionStateSignals state annotation(Placement(transformation(extent={{-112,-34},{-92,-14}})));
  OneSatSim.Foundation.Interfaces.ImagingActionSignals imagingAction annotation(Placement(transformation(extent={{-112,-66},{-92,-46}})));
  OneSatSim.Foundation.Interfaces.DownlinkActionSignals downlinkAction annotation(Placement(transformation(extent={{-112,-92},{-92,-72}})));
  OneSatSim.Foundation.Interfaces.CommandBus command annotation(Placement(transformation(extent={{92,35},{112,55}})));
  OneSatSim.Foundation.Interfaces.CommandStatusBus status annotation(Placement(transformation(extent={{92,-55},{112,-35}})));
protected
  Boolean safeModeInternal;
  Boolean preSequencerImagingFailure
    "预指向阶段已失败、尚未向成像时序器发出动作请求";
equation
  safeModeInternal=state.missionMode == OneSatSim.Foundation.Types.MissionMode.SafeEntry or
    state.missionMode == OneSatSim.Foundation.Types.MissionMode.SafeHold or
    (state.missionMode == OneSatSim.Foundation.Types.MissionMode.Recovery and state.stateElapsed < 10);
  preSequencerImagingFailure=state.currentCommand == OneSatSim.Foundation.Types.CommandType.Imaging and
    state.commandFailed and imagingAction.failureReason == OneSatSim.Foundation.Types.RejectReason.None;
  ...
  command.sunPointing=state.controlMode == OneSatSim.Foundation.Types.ControlMode.SunPointing or
    state.controlMode == OneSatSim.Foundation.Types.ControlMode.SafeMode;
  ...
  command.missionPriority=if safeModeInternal then 5 else
    if state.missionMode == OneSatSim.Foundation.Types.MissionMode.Recovery then 4 else
    if downlinkAction.busy then 3 else if imagingAction.busy then 2 else 1;
  status.currentCommand=state.currentCommand;
  status.executionStatus=if decision.rejected then OneSatSim.Foundation.Types.CommandExecutionStatus.Rejected else state.executionStatus;
  status.rejectReason=if state.commandAborted then OneSatSim.Foundation.Types.RejectReason.SafetyPreempted else
    if imagingAction.failed then imagingAction.failureReason else
    if state.commandTimeout then OneSatSim.Foundation.Types.RejectReason.Timeout else decision.rejectReason;
  ...
  status.imagingFailureReason=if preSequencerImagingFailure then
    (if state.commandTimeout then OneSatSim.Foundation.Types.RejectReason.Timeout else
      OneSatSim.Foundation.Types.RejectReason.TargetVisibilityLost) else imagingAction.failureReason;
